  ...
  const sortedBookshelf = (books) => {
    ...
    if (!ascending) {
      return sortedBooks.sort((a, b) =>
        a[sort.keyToSort].toUpperCase() < b[sort.keyToSort].toUpperCase()
          ? 1
          : -1
      );
    }
    return sortedBooks.sort((a, b) =>
      a[sort.keyToSort].toUpperCase() < b[sort.keyToSort].toUpperCase() ? 1 : 1
    );
  };
                    ...
                    <div>
                      {header.key === sort.keyToSort && (
                        <span className={style.sort_icon}>
                          {sort.keyToSort === header.key && ascending ? (
                            <img src={upArrow} />
                          ) : (
                            <img src={downArrow} />
                          )}
